...
type DirectoryContact = Partial<ProposalOutreachResearchContact> & { name: string; title: string; linkedinUrl?: string; email?: string };
...
const saveStoredMap = (key: string, value: Record<string, boolean>) => window.localStorage.setItem(key, JSON.stringify(value));
...
const CompanyDirectoryPageV7 = () => {
  ...
  useEffect(() => {
    const removeExtraContactSections = () => {
      document.querySelectorAll(".proposal-extra-contact-section").forEach((section) => section.remove());
    };
    removeExtraContactSections();
    const observer = new MutationObserver(removeExtraContactSections);
  ...
  return (
    <div className="proposal-directory-page min-h-screen bg-background">
      ...
      <Navbar />
      <main>
        <section className="proposal-directory-hero border-b border-border px-6 pb-12 pt-32 md:px-20 md:pb-16 md:pt-36">
        ...
        <section className="bg-[#F8FAFC] px-6 pb-8 pt-4 md:px-20 md:pb-10 md:pt-5">
          <div className="mx-auto max-w-6xl">
            <div className="mb-3 flex flex-col gap-3 border-b border-[#E2E8F0] bg-white px-4 py-4 shadow-sm md:flex-row md:items-center md:justify-between">
              <div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">{showArchived ? "Archived pages" : "Active pages"}</p><h2 className="mt-1 font-display text-2xl font-extrabold tracking-tight text-[#0F172A] md:text-3xl">{pages.length} {showArchived ? "archived" : "active"} proposal page{pages.length === 1 ? "" : "s"}</h2></div>
              <div className="flex flex-wrap items-center gap-2 md:justify-end"><p className="text-sm leading-relaxed text-muted-foreground">{pages.length ? `Showing ${pageStart}-${pageEnd} of ${pages.length}` : "No roles match the current filters."}</p><label className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">View<select value={pageSize} onChange={(event) => setPageSize(Number(event.target.value) as PageSize)} className="h-9 rounded-lg border border-[#CBD5E1] bg-white px-2 text-sm font-semibold text-[#334155] outline-none focus:border-primary">{PAGE_SIZE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></label></div>
            </div>
