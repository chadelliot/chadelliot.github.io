  ...
  return (
    <div className="campaign">
      ...
      <div className="campaign-grid">
        {[
          ["Audience", "Growth pharma and biotech teams preparing launches or indication expansions", "Launch prep"],
          ["Message", "Identify market access, field, and stakeholder friction before launch execution", "Friction before launch"],
          ["Channels", "LinkedIn account targeting, display retargeting, executive email, SDR outreach, workshop invite", "Multi-channel"],
          ["Sales Handoff / Measurement", "Workshop conversion, SQLs, proposal rate, pipeline value, executive dashboard adoption", "Proof and adoption"],
        ].map(([label, value, tag]) => (
          <div className="campaign-cell activation-card" key={label}>
            <div className="rg-layer">{label}</div>
            <div className="card-title">{value}</div>
            <div className="card-body">{label === "Sales Handoff / Measurement" ? "Measure the response, the pipeline value, and executive adoption." : "Keep the activation tight and tied to launch readiness."}</div>
            <span className="tag">{tag}</span>
          </div>
        ))}
      </div>
